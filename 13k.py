...
try:
    ...
    PIL_AVAILABLE = False
    print("Предупреждение: PIL (Pillow) не установлен. Работа с изображениями будет ограничена.")
...
try:
    ...
    print("Предупреждение: pandas не установлен. Импорт/экспорт Excel недоступен.")
...
try:
    ...
    print("Предупреждение: reportlab не установлен. Экспорт в PDF недоступен.")
...
try:
    ...
    print("Предупреждение: openpyxl не установлен. Экспорт в Excel будет ограничен.")
    print("Установите: pip install openpyxl")

# Класс ModernDatabaseApp и все остальные классы остаются без изменений...
...
class ModernDatabaseApp(QMainWindow):
    ...
    def initUI(self):
        ...
        hotkeys_label = QLabel("🔥 Горячие клавиши: F5=Обновить, Ctrl+S=Сохранить, Del=Удалить, Ctrl+P=Печать, Ctrl+I=Добавить фото")
        ...
        actions = [
            ...
            ("📸 Добавить фото", self.quick_add_photo, "warning")  # НОВАЯ КНОПКА!
        ]
    ...
    def setup_hotkeys(self):
        ...
        # Ctrl+I - быстрое добавление фото (НОВАЯ КЛАВИША!)
        photo_shortcut = QShortcut(QKeySequence("Ctrl+I"), self)
        photo_shortcut.activated.connect(self.quick_add_photo)
    ...
    def quick_add_photo(self):
        ...
        if not self.is_image_column(column_name):
            # Спрашиваем пользователя, хочет ли он добавить фото в обычную колонку
            reply = QMessageBox.question(
                self,
                "Подтверждение",
                f"Колонка '{column_name}' не определена как колонка для фото.\n"
                "Хотите добавить фото в эту колонку?",
        ...
        # Открываем диалог добавления фото
    ...
    def get_column_info(self, display_column_name):
        """Получение информации о столбце по отображаемому имени"""
        # Извлекаем чистое имя столбца
        clean_name = display_column_name
        if '.' in display_column_name:
            clean_name = display_column_name.split('.')[-1]

        # Ищем в отображении
        if clean_name in self.column_mapping:
            return self.column_mapping[clean_name]

        # Если не нашли в отображении, ищем в таблицах
        try:
            cursor = self.connection.cursor()
            ...
            for col in columns:
                if col[1] == clean_name:
                    sql_name = f"{self.escape_table_name(self.current_table)}.{self.escape_table_name(clean_name)}"
                    return {
                        'sql_name': sql_name,
            ...
            # Проверяем присоединенные таблицы
            for join_info in self.joined_tables:
                ...
                for col in columns:
                    if col[1] == clean_name:
                        sql_name = f"{self.escape_table_name(table_name)}.{self.escape_table_name(clean_name)}"
                        return {
                            'sql_name': sql_name,
                            'table': table_name,
                            'clean_name': clean_name
    ...
    def build_query(self, sort_column=None, sort_order="ASC"):
        ...
        used_columns = set()
        select_columns = []
        self.column_mapping = {}  # Инициализируем отображение имен
            ...
            try:
                ...
                for col in columns:
                    ...
                    if col_name not in used_columns:
                        sql_name = f"{self.escape_table_name(table_name)}.{self.escape_table_name(col_name)}"
                        select_columns.append(sql_name)
                        # Сохраняем отображение: чистое имя -> полное SQL имя
        ...
        if self.selected_attributes:
            final_columns = []
            used_columns.clear()
            self.column_mapping = {}  # Сбрасываем отображение для выбранных атрибутов
            for attr in self.selected_attributes:
                if '.' in attr:
                    table, col = attr.split('.')
                    if col not in used_columns:
                        sql_name = f"{self.escape_table_name(table)}.{self.escape_table_name(col)}"
                        final_columns.append(sql_name)
                    ...
                    if attr not in used_columns:
                        sql_name = self.escape_table_name(attr)
                        final_columns.append(sql_name)
                        # Сохраняем отображение
                        self.column_mapping[attr] = {
                            'sql_name': sql_name,
                            'table': self.current_table,  # Предполагаем основную таблицу
                            'clean_name': attr
                        }
    ...
    def show_context_menu(self, position):
        ...
        selected_items = self.table_widget.selectedItems()
        if not selected_items:
            # Если ничего не выбрано, показываем пустое меню
            return
        ...
        # Проверяем, является ли это фото-колонкой
        is_image_column = self.is_image_column(column_name)

        # Проверяем, есть ли фото в ячейке сейчас
        item = self.table_widget.item(row, col)
        has_photo = False
        if item and item.text() == "🖼️ Фото":
            has_photo = True
        elif item and item.data(Qt.ItemDataRole.UserRole):
            # Проверяем, есть ли данные фото
            has_photo = True
        ...
        # Действия с фото (если это фото-колонка)
        if is_image_column:
            if has_photo:
                # Если фото уже есть
                view_photo_action = menu.addAction("🖼️ Просмотреть фото")
                change_photo_action = menu.addAction("📷 Изменить фото")
                remove_photo_action = menu.addAction("🗑️ Удалить фото")
            else:
                # Если фото нет
                add_photo_action = menu.addAction("📷 Добавить фото")
        ...
        # Обработка действий с фото
            ...
            if has_photo:
                if action == view_photo_action:
                    self.view_selected_image_full()
                elif action == change_photo_action:
                    self.add_photo_dialog(column_name, row, col)
                elif action == remove_photo_action:
                    self.remove_photo_from_cell(row, col, column_name)
            else:
                if action == add_photo_action:
                    self.add_photo_dialog(column_name, row, col)

    def on_cell_double_click(self, index):
        """Обработка двойного клика по ячейке"""
        # Используем переданный индекс вместо selectedItems()
        row = index.row()
        col = index.column()
        ...
        if not column_info:
            QMessageBox.warning(self, "Ошибка", f"Не удалось найти информацию о столбце '{column_name}'")
            return

        table_name = column_info['table']
        clean_column_name = column_info['clean_name']

        col_type = self.get_column_type(table_name, clean_column_name)

        if col_type and col_type.upper() == 'BOOLEAN':
            dialog = BooleanEditDialog(self, column_name, current_value)
            if dialog.exec():
                new_value = dialog.get_value()
                self.update_cell_value(row, col, new_value, table_name, clean_column_name)
        else:
            ...
            if ok and text != current_value:
                self.update_cell_value(row, col, text, table_name, clean_column_name)
    ...
    def remove_photo_from_cell(self, row, col, column_name):
        """Удаление фото из ячейки"""
        reply = QMessageBox.question(
            self,
            "Удаление фото",
            "Вы уверены, что хотите удалить фото из этой ячейки?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                # Находим колонку с первичным ключом в отображаемых данных
                pk_col = -1
                ...
                # Обновляем базу данных - устанавливаем NULL
                query = f"""
                    UPDATE {self.escape_table_name(self.current_table)} 
                    SET {self.escape_table_name(column_name)} = NULL 
                ...
                if item:
                    ...
                    item.setData(Qt.ItemDataRole.UserRole, None)  # Удаляем данные фото

                self.update_status("✅ Фото удалено из ячейки!")

            except sqlite3.Error as e:
                QMessageBox.critical(self, "Ошибка", f"Ошибка при удалении фото: {e}")
    ...
    def get_column_table(self, column_name):
        ...
        # Извлекаем чистое имя столбца (убираем префикс таблицы если есть)
        clean_column_name = column_name
        if '.' in column_name:
            clean_column_name = column_name.split('.')[-1]

        try:
            # Сначала проверяем основную таблицу
            cursor = self.connection.cursor()
            cursor.execute(f"PRAGMA table_info({self.escape_table_name(self.current_table)})")
            columns = cursor.fetchall()
            for col in columns:
                if col[1] == clean_column_name:
                    return self.current_table
        except sqlite3.Error:
            pass

        # Затем проверяем присоединённые таблицы
        for join_info in self.joined_tables:
            table_name = join_info['table2']
            try:
                cursor = self.connection.cursor()
        ...
        # Если не нашли, возвращаем основную таблицу по умолчанию
        return self.current_table
    ...
    def build_query(self, sort_column=None, sort_order="ASC"):
        ...
        used_columns = set()
        select_columns = []
        column_mapping = {}  # NEW: Отображение чистого имени -> полное SQL имя
            ...
            try:
                ...
                for col in columns:
                    ...
                    if col_name not in used_columns:
                        sql_name = f"{self.escape_table_name(table_name)}.{self.escape_table_name(col_name)}"
                        select_columns.append(sql_name)
                        column_mapping[col_name] = sql_name  # Сохраняем отображение
                        used_columns.add(col_name)
            except sqlite3.Error:
                pass
            ...
            for attr in self.selected_attributes:
                if '.' in attr:
                    ...
                    if col not in used_columns:
                        sql_name = f"{self.escape_table_name(table)}.{self.escape_table_name(col)}"
                        final_columns.append(sql_name)
                        column_mapping[col] = sql_name  # Сохраняем отображение
                        used_columns.add(col)
                else:
                    if attr not in used_columns:
                        final_columns.append(self.escape_table_name(attr))
                        column_mapping[attr] = self.escape_table_name(attr)  # Сохраняем отображение
                        used_columns.add(attr)
            select_columns = final_columns
        ...
        # Для отображения используем чистые имена столбцов
        display_columns = []
        for sql_col in select_columns:
            clean_col = sql_col.replace('"', '')
            if '.' in clean_col:
                display_columns.append(clean_col.split('.')[-1])
            else:
                display_columns.append(clean_col)
    ...
    def get_sql_column_name(self, display_column_name):
        """Получение SQL имени столбца по отображаемому имени"""
        # Убираем префикс таблицы если есть
        clean_name = display_column_name
        if '.' in display_column_name:
            clean_name = display_column_name.split('.')[-1]

        # Восстанавливаем SQL имя из отображения, созданного в build_query
        # Для этого нужно сохранять mapping при построении запроса
        # Простой подход: ищем столбец во всех таблицах
        try:
            cursor = self.connection.cursor()
            ...
            for col in columns:
                if col[1] == clean_name:
                    return f"{self.escape_table_name(self.current_table)}.{self.escape_table_name(clean_name)}"
            ...
            for join_info in self.joined_tables:
                ...
                for col in columns:
                    if col[1] == clean_name:
                        return f"{self.escape_table_name(table_name)}.{self.escape_table_name(clean_name)}"

            # Если не нашли, возвращаем простое имя
            return self.escape_table_name(clean_name)

        except sqlite3.Error:
            return self.escape_table_name(clean_name)

    def update_cell_value(self, row, col, new_value, table_name, clean_column_name):
        """Обновление значения ячейки в базе данных"""
        try:
            cursor = self.connection.cursor()

            # Получаем все столбцы выбранной таблицы
            cursor.execute(f"PRAGMA table_info({self.escape_table_name(table_name)})")
            columns_info = cursor.fetchall()

            # Находим первичный ключ для ЭТОЙ таблицы
            primary_key_name = None
            for col_info in columns_info:
                if col_info[5]:  # Пятый элемент в PRAGMA table_info - это флаг первичного ключа
                    primary_key_name = col_info[1]
                    break

            # Если первичный ключ не найден, берем первый столбец
            if not primary_key_name and columns_info:
                primary_key_name = columns_info[0][1]
            ...
            # Ищем колонку с первичным ключом в отображаемых данных
            # Ищем столбец с таким же именем, как первичный ключ таблицы table_name
            pk_col = -1
            for i in range(self.table_widget.columnCount()):
                header_text = self.table_widget.horizontalHeaderItem(i).text()
                clean_header = header_text.split('.')[-1] if '.' in header_text else header_text
                # Проверяем, что это первичный ключ И таблица соответствует
                if clean_header == primary_key_name:
                    # Проверяем, что это из нужной таблицы
                    column_info = self.get_column_info(header_text)
                    if column_info and column_info['table'] == table_name:
                        pk_col = i
                        break

            if pk_col == -1:
                QMessageBox.critical(self, "Ошибка",
                                     f"Не удалось найти столбец '{primary_key_name}' из таблицы '{table_name}' в отображаемых данных!")
                return

            # Получаем значение первичного ключа
            pk_item = self.table_widget.item(row, pk_col)
            if not pk_item:
                QMessageBox.critical(self, "Ошибка", f"Не удалось получить значение первичного ключа из строки {row}")
                return

            primary_key_value = pk_item.text()
            ...
            if col_type and col_type.upper() == 'BOOLEAN':
                if isinstance(new_value, str):
                    if new_value.lower() in ['true', '1', 'да', 'yes', 'истина']:
                        processed_value = 1
                    elif new_value.lower() in ['false', '0', 'нет', 'no', 'ложь']:
                        processed_value = 0
                    else:
                        processed_value = new_value
                else:
                    processed_value = 1 if new_value else 0
            elif col_type and col_type.upper() in ['INTEGER', 'REAL']:
                try:
                    processed_value = float(new_value) if '.' in new_value else int(new_value)
                except ValueError:
                    processed_value = new_value

            # Формируем и выполняем запрос
            query = f"""
                UPDATE {self.escape_table_name(table_name)} 
                SET {self.escape_table_name(clean_column_name)} = ? 
                WHERE {self.escape_table_name(primary_key_name)} = ?
            """
